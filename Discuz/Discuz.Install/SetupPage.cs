...
namespace Discuz.Install
{
    ...
    //用__doPostBack来提交，保证按钮的服务器端click事件执行
        ...
        public static string InitialSystemValidCheck(ref bool error)
        {
            ...
            //			GetRootDntconfigPath();
            //检查Dnt.config文件的有效性
            if (!GetRootDntconfigPath())
            {
                sb.Append("<tr style=\"height:15px\"><td bgcolor='#ffffff' width='5%'><img src='images/error.gif' width='16' height='16'></td><td bgcolor='#ffffff' width='95%'> DNT.config 不可写或没有放置正确, 相关问题详见安装文档!</td></tr>");
                error = true;
            }
            else
            {
                sb.Append("<tr style=\"height:15px\"><td bgcolor='#ffffff' width='5%'><img src='images/ok.gif' width='16' height='16'></td><td bgcolor='#ffffff' width='95%'>对 DNT.config 验证通过!</td></tr>");
            }

            //			if(!SystemRootCheck())
            //			{
            //				sb.Append("您对系统文件dnt.config没有写入权限!<br />");
            //			}
        ...
        public static bool GetRootDntconfigPath()
        {
            try
            {
                ...
                //string[] webconfigpath = {Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "DNT.config"), Path.Combine(context.Request.PhysicalApplicationPath, "dnt.config"),
                //    Utils.GetMapPath("../dnt.config") ,Utils.GetMapPath("../../dnt.config"),Utils.GetMapPath("../../../dnt.config") };
                //foreach (string webconfig in webconfigpath)
                //{
                //    if (Utils.FileExists(webconfig))
                //    {
                //        return SystemFileCheck(webconfig);
                //    }
                //}
                //return false;
                //如果文件不存在退出
        ...
        public static string IISSystemBINCheck(ref bool error)
        {
            ...
            try
            {
                string[] assemblylist = new string[] { "Discuz.Aggregation.dll", "Discuz.Cache.dll", "Discuz.Common.dll", "Discuz.Config.dll", 
                    ...
                    "Discuz.Install.dll", "Discuz.Plugin.dll","Discuz.Plugin.Preview.Jpg.dll","Discuz.Plugin.Spread.dll", "Discuz.Web.Admin.dll",
