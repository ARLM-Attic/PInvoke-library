...
namespace Discuz.Mall.Pages
{
    ...
    public class showgoods : PageBase
    {
        ...
        protected override void ShowPage()
        {
            ...
            {
                recommendgoodslist = Goods.GetGoodsRecommendList(goodsinfo.Selleruid,
                    6,
                    1,
            ...
            if (DNTRequest.GetInt("deletegoods", 0) == 1)
            {
                ...
                {
                    AddErrLine("你不是当前商品的卖家或版主，因些无法删除该商品");
                    return;
                }
            }                     
            ...
            if (ispost)
            {
                ...
                if (DNTRequest.GetString("postleaveword") == "add")
                {
                    ...
                    if (GoodsLeaveWords.CreateLeaveWord(goodsleavewordinfo) > 0)
                    {
                        //发送短消息给卖家
                        if (goodsleavewordinfo.Isbuyer == 1)
                        {
                            PrivateMessageInfo __privatemessageinfo = new PrivateMessageInfo();

                            // 收件箱
                            __privatemessageinfo.Subject = Utils.HtmlEncode("[系统消息] 用户给您商品留言");
                            __privatemessageinfo.Message = Utils.HtmlEncode("用户 " + goodsleavewordinfo.Username + " 给您商品 " + goodsinfo.Title + " 留言, 请<a href =\"" + base.ShowGoodsAspxRewrite(goodsleavewordinfo.Goodsid) + "\" target=\"_blank\">点击这里</a>查看详情. ");
                            __privatemessageinfo.Msgto = goodsinfo.Seller;
                            __privatemessageinfo.Msgtoid = goodsinfo.Selleruid;
