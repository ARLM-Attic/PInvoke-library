...
namespace Discuz.Mall.Pages
{
    ...
    public class goodsadmin : PageBase
    {
        ...
        protected override void ShowPage()
        {
            ...
            goodslist = DNTRequest.GetString("goodsid");
        ...
        private bool DoOperations(ForumInfo forum, AdminGroupInfo admininfo, int reasonpm)
        {
            ...
            if (
                "delete,highlight,close,movecategory"
                    .IndexOf(operation) == -1)
            {
                AddErrLine("未知的操作参数");
                return false;
        ...
        private bool DoCloseOperation()
        {
            ...
            close = 1;// DNTRequest.GetFormInt("close", -1);
            //if (close == -1)
            //{
            //    //
            //    AddErrLine("您没有选择操作类型(关闭)");
            //    return false;
            //}
