...
namespace Discuz.Mall.Pages
{
    ...
    public class editgoods : PageBase
    {
        ...
        protected override void ShowPage()
        {
            ...
            canhtmltitle = true;// config.Htmltitle == 1 && Utils.InArray(usergroupid.ToString(), config.Htmltitleusergroup);
            ...
            else
            {
                ...
                if (admininfo != null && admininfo.Disablepostctrl != 1)
                {
                    ...
                    {
                        AddErrLine("您发表的内容过少, 系统设置要求商品内容不得少于 " + config.Minpostsize.ToString() + " 字多于 " + config.Maxpostsize.ToString() + " 字");
                    }
                    else if (postmessage.Length > config.Maxpostsize)
                    {
                        AddErrLine("您发表的内容过多, 系统设置要求商品内容不得少于 " + config.Minpostsize.ToString() + " 字多于 " + config.Maxpostsize.ToString() + " 字");
                    }
                }
