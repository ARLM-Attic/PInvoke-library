...
*/
                ...
                if (_postpramsinfo.Bbcodemode == 1)
                {
                    ...
                    for (m = r[5].Match(sDetail); m.Success; m = m.NextMatch())
                    {
                        ...
                        replacement = "<p><img alt=\"\" src=\"images/attachicons/attachment.gif\" border=\"0\" /><span class=\"bold\">附件</span>: <a href=\"" + BaseConfigs.GetForumPath + m.Groups[2].ToString().ToLower().Replace("viewfile.asp?id", "attachment.aspx?attachmentid") + "\" target=\"_blank\">" + replacement + "</a> </p>";
                        sDetail = sDetail.Replace(m.Groups[0].ToString(), replacement);
