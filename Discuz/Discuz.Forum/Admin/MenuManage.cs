...
namespace Discuz.Forum
{
    public class MenuManage
    {
        ...
        /*public static bool UpSubMenu(int submenuid, int mainmenuid)
        {
            XmlDocumentExtender doc = new XmlDocumentExtender();
            doc.Load(configPath);
            XmlNodeList submains = doc.SelectNodes("/dataset/mainmenu");
            string oldmenuid = "", newmenuid = "";
            for (int i = 0; i < submains.Count; i++ )
            {
                if(submains[i]["id"].InnerText == submenuid.ToString())
                {
                    oldmenuid = submains[i]["menuid"].InnerText;
                    newmenuid = submains[i - 1]["menuid"].InnerText;
                    string upmenutitle = submains[i]["menutitle"].InnerText;
                    submains[i]["menutitle"].InnerText = submains[i - 1]["menutitle"].InnerText;
                    submains[i - 1]["menutitle"].InnerText = upmenutitle;
                    break;
                }
            }
            XmlNodeList items = doc.SelectNodes("/dataset/submain");
            foreach(XmlNode item in items)
            {
                if(item["menuparentid"].InnerText == newmenuid)
                {
