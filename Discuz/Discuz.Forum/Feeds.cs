...
namespace Discuz.Forum
{
	...
	public class Feeds
	{
		...
		public static string GetRssXml(int ttl)
		{
			...
			string str = cache.RetrieveObject("/Forum/RSS/Index") as string;
			if (str != null)
			{
				return str;
			}
			...
			else
			{
				...
				cache.AddObject("/Forum/RSS/Index", sbRss.ToString());
		...
		public static string GetForumRssXml(int ttl, int fid)
		{
			...
			string str = cache.RetrieveObject("/Forum/RSS/Forum" + fid) as string;
			if(str != null)
			{
				return str;
			}

            string forumurl = "http://" + DNTRequest.GetCurrentFullHost() + BaseConfigs.GetForumPath.ToLower();//GeneralConfigs.GetConfig().Forumurl;
            ...
            ics.TimeOut = ttl;
            cache.LoadCacheStrategy(ics);
			cache.AddObject("/Forum/RSS/Forum" + fid, sbRss.ToString());
			cache.LoadDefaultCacheStrategy();
        ...
        public static string GetBaiduSitemap(int ttl)
        {
            ...
            string str = cache.RetrieveObject("/Forum/Sitemap/Baidu") as string;
            if (str != null)
            {
                return str;
            }
            ...
            else
            {
                ...
                ics.TimeOut = ttl * 60;
                cache.LoadCacheStrategy(ics);
                cache.AddObject("/Forum/Sitemap/Baidu", sitemapBuilder.ToString());
                cache.LoadDefaultCacheStrategy();
