...
namespace Discuz.Data.Access
{
    public partial class DataProvider : IDataProvider
    {
        ...
        {
            DbParameter[] prams = {
										   DbHelper.MakeInParam("@starttime",(DbType)OleDbType.VarChar,19, starttime),
										   DbHelper.MakeInParam("@endtime",(DbType)OleDbType.VarChar,19, endtime)
									   };
        ...
        public void ShrinkDataBase(string shrinksize, string dbname)
        {
            ...
            //SqlString += "SELECT @StartTime = now(), ";
            ...
            //SqlString += "WHILE @MaxMinutes > DATEDIFF (mi, @StartTime, now()) -- time has not expired ";
        ...
        public string GetSpecialTableFullIndexSQL(string tablename)
        {
            ...
            //sb.Append("CONSTRAINT [DF_" + tablename + "_postdatetime] DEFAULT (now()) FOR [postdatetime],CONSTRAINT [DF_" + tablename + "_message] DEFAULT ('') FOR [message],");
        ...
        public int AddTemplate(string templateName, string directory, string copyright)
        {
            ...
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, "INSERT INTO [" + BaseConfigs.GetTablePrefix + "templates]([templatename],[directory],[copyright]) VALUES(@templatename, @directory, @copyright);", prams), -1);
        }
        ...
        {
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, "SELECT iff(ISNULL(MAX(id)), 0,MAX(id)) FROM " + BaseConfigs.GetTablePrefix + "smilies"), 0) + 1;
        }
        ...
        {
            return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT SUM([topics]) AS [topiccount],SUM([posts]) AS [postcount],SUM([todayposts])-(SELECT SUM([todayposts]) FROM [{0}forums] WHERE [lastpost] < CONVERT(CHAR(12),now(),101) AND [layer]=1) AS [todaypostcount] FROM [{0}forums] WHERE [layer]=1", BaseConfigs.GetTablePrefix));
        }

        public IDataReader GetForumStatistics(int fid)
        {
            DbParameter[] prams = { DbHelper.MakeInParam("@fid", (DbType)OleDbType.Integer,4, fid) };
            return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT SUM([topics]) AS [topiccount],SUM([posts]) AS [postcount],SUM([todayposts])-(SELECT SUM([todayposts]) FROM [{0}forums] WHERE [lastpost] < CONVERT(CHAR(12),now(),101) AND [layer]=1 AND [fid] = @fid) AS [todaypostcount] FROM [{0}forums] WHERE [fid] = @fid AND [layer]=1", BaseConfigs.GetTablePrefix), prams);
        }
        ...
        public DateTime GetPostStartTime()
        {
            ...
            string commandText = string.Format("SELECT MIN([postdatetime]) FROM [{0}" + currenttable + "] WHERE [invisible]=0", BaseConfigs.GetTablePrefix);
        ...
        public IDataReader GetBestMember(string posttableid)
        {
            ...
            string commandText = string.Format("SELECT TOP 1 [poster], COUNT(1) AS [posts] FROM [{0}posts{1}] WHERE [postdatetime]>=#{2}# AND [invisible]=0 AND [posterid]>0 GROUP BY [poster] ORDER BY COUNT(1) DESC", BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.ToString("yyyy-MM-dd"));
            return DbHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        public IDataReader GetDayPostsStats(string posttableid)
        {
            ...
            string commandText = string.Format("SELECT COUNT(1) AS [count],YEAR([postdatetime]) AS [year], MONTH([postdatetime]) AS [month], DAY([postdatetime]) AS [day] FROM [{0}posts{1}] WHERE [invisible]=0 AND [postdatetime] > #{2}# GROUP BY YEAR([postdatetime]), MONTH([postdatetime]),DAY([postdatetime]) ORDER BY YEAR([postdatetime]),MONTH([postdatetime]),DAY([postdatetime])", BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.AddDays(-30).ToString("yyyy-MM-dd"));
            return DbHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        public IDataReader GetUsersRank(int count, string posttableid, string type)
        {
            ...
            switch(type)
            {
                ...
                case "digestposts":
                    ...
                    break;
                case "thismonth":
                    commandText = string.Format("SELECT DISTINCT TOP {0} [poster] AS [username], [posterid] AS [uid], COUNT(pid) AS [posts] FROM [{1}posts{2}] WHERE [postdatetime]>=#{3}# AND [invisible]=0 AND [posterid]>0 GROUP BY [poster], [posterid] ORDER BY COUNT(pid) DESC", count, BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.AddDays(-30).ToString("yyyy-MM-dd"));
                    break;
                case "today":
                    commandText = string.Format("SELECT DISTINCT TOP {0} [poster] AS [username], [posterid] AS [uid], COUNT(pid) AS [posts] FROM [{1}posts{2}] WHERE [postdatetime]>=#{3}# AND [invisible]=0 AND [posterid]>0 GROUP BY [poster], [posterid] ORDER BY COUNT(pid) DESC", count, BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.ToString("yyyy-MM-dd"));
                    break;
