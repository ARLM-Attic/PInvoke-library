    ...
    public class RESTServer : Page
    {
        ...
        void RESTServer_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                callid = Util.GetFloatFromString(DNTRequest.GetString("call_id"));
                if (callid > -1)
                {
                    lastcallid = Session["call_id"] != null ? Util.GetFloatFromString(Session["call_id"].ToString()) : -1;
                }
        ...
        private void ResponseErrorInfo(int errorCode)
        {
            ...
            {
                list.Add(new Arg(key, Request.QueryString[key]));
            }
            foreach (string key in Request.Form.AllKeys)
            {
                list.Add(new Arg(key, Request.Form[key]));
            }
            if (list.Count > 0)
            {
            ...
            string responseStr = SerializationHelper.Serialize(error);
        ...
        private DNTParam[] GetParamsFromRequest(HttpRequest request)
        {
            ...
            list.Sort();
#if NET1
            return (DNTParam[])list.ToArray(typeof(DNTParam));
#else
            return list.ToArray();
#endif
        }
