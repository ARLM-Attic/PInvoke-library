...
namespace Discuz.Data.MySql
{
    public partial class DataProvider : IDataProvider
    {
        ...
        {
            DbParameter parm = DbHelper.MakeInParam("?lastuserid", (DbType)MySqlDbType.Int32, 4, lastuserid);
 ...
 ") ENGINE = MYISAM CHARACTER SET gbk COLLATE gbk_chinese_ci";
        ...
        {
            DbParameter parm = DbHelper.MakeInParam("?aid", (DbType)MySqlDbType.Int32, 4, aid);
        ...
        public int AddPaymentLog(int uid, int tid, int posterid, int price, float netamount)
        {
            DbParameter[] prams = {
									   ...
									   DbHelper.MakeInParam("?buydate",(DbType)MySqlDbType.Datetime,4,DateTime.Now),
									   DbHelper.MakeInParam("?amount",(DbType)MySqlDbType.Int32,4,price),
									   DbHelper.MakeInParam("?netamount",(DbType)MySqlDbType.Float,8,netamount)
								   };
        ...
        {
            DbParameter parm = DbHelper.MakeInParam("?type", (DbType)MySqlDbType.Int32, 4, type);
        ...
        {
            DbParameter parm = DbHelper.MakeInParam("?pid", (DbType)MySqlDbType.Int32, 4, pid);
        ...
        public IDataReader GetTagsListByTopic(int topicid)
        {
            ...
            DbParameter parm = DbHelper.MakeInParam("?topicid", (DbType)MySqlDbType.Int32, 4, topicid);
        ...
        {
            //string sql1 = string.Format("DELETE FROM [{0}scheduledevents] WHERE ([key]=@key) AND ([lastexecuted] < DATEADD([day], - 7, NOW()))", BaseConfigs.GetTablePrefix);
            string sql1 = string.Format("DELETE FROM `{0}scheduledevents` WHERE (`key`=?key) AND (`lastexecuted` < ADDDATE( NOW(),- 7))", BaseConfigs.GetTablePrefix);
            string sql2 = string.Format("INSERT `dnt_scheduledevents` (`key`, `servername`, `lastexecuted`) Values (?key, ?servername, ?lastexecuted)");
            DbParameter[] parms = {
                DbHelper.MakeInParam("?key", (DbType)MySqlDbType.VarChar, 100, key),
                DbHelper.MakeInParam("?servername", (DbType)MySqlDbType.VarChar, 100, servername),
                DbHelper.MakeInParam("?lastexecuted", (DbType)MySqlDbType.Datetime, 8, lastexecuted)
            };
        ...
        public DateTime GetLastExecuteScheduledEventDateTime(string key, string servername)
        {
            ...
            DbParameter[] parms = {
                DbHelper.MakeInParam("?key", (DbType)MySqlDbType.VarChar, 100, key),
                DbHelper.MakeInParam("?servername", (DbType)MySqlDbType.VarChar, 100, servername)
                //DbHelper.MakeOutParam("@lastexecuted", (DbType)MySqlDbType.Datetime, 8)
            };
            ...
            string t= DbHelper.ExecuteScalar(CommandType.Text,"SELECT MAX(`lastexecuted`) FROM `"+BaseConfigs.GetTablePrefix+"scheduledevents` WHERE `key` = ?key AND `servername` = ?servername",parms).ToString();
            if (t ==null || t=="")
            {
                t = DbHelper.ExecuteScalar(CommandType.Text, "select  ADDDATE(now(),-365)").ToString();
        ...
        {
            DbParameter[] parms = {
                DbHelper.MakeInParam("?type", (DbType)MySqlDbType.VarChar, 10, type),
                DbHelper.MakeInParam("?variable", (DbType)MySqlDbType.VarChar, 20, variable),
        ...
        {
            DbParameter[] parms = {
                DbHelper.MakeInParam("?type", (DbType)MySqlDbType.VarChar, 20, type),
                DbHelper.MakeInParam("?variable", (DbType)MySqlDbType.VarChar, 20, variable),
                DbHelper.MakeInParam("?value", (DbType)MySqlDbType.Text, 0, value)
            };
        ...
        {
            DbParameter parm = DbHelper.MakeInParam("?type", (DbType)MySqlDbType.VarChar, 10, type);
        ...
        public IDataReader GetBestMember(string posttableid)
        {
            ...
            string commandText = string.Format("SELECT `poster`, COUNT(1) AS `posts` FROM `{0}posts{1}` WHERE `postdatetime`>='{2}' AND `invisible`=0 AND `posterid`>0 GROUP BY `poster` ORDER BY `posts` DESC limit 1", BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.ToString("yyyy-MM-dd"));
            return DbHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        {
            string commandText = string.Format("SELECT `fid`, `name`, `topics` FROM `{0}forums` WHERE `status`>0 AND `layer`=0 ORDER BY `topics` DESC LIMIT {0}", BaseConfigs.GetTablePrefix, count);
            return DbHelper.ExecuteReader(CommandType.Text, commandText);
        }

        public IDataReader GetForumsByPostCount(int count)
        {
            string commandText = string.Format("SELECT `fid`, `name`, `posts` FROM `{0}forums` WHERE `status`>0 AND `layer`=0 ORDER BY `posts` DESC LIMIT {0}", BaseConfigs.GetTablePrefix, count);
            return DbHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        public IDataReader GetForumsByMonthPostCount(int count, string posttableid)
        {
            ...
            string commandText = string.Format("SELECT DISTINCT `p`.`fid`, `f`.`name`, COUNT(`pid`) AS `posts` FROM `{0}posts{1}` `p` LEFT JOIN `{0}forums` `f` ON `p`.`fid`=`f`.`fid` WHERE `postdatetime`>='{2}' AND `invisible`=0 AND `posterid`>0 GROUP BY `p`.`fid`, `f`.`name` ORDER BY `posts` DESC limit {3}", BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.AddDays(-30).ToString("yyyy-MM-dd"), count);
            return DbHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        public IDataReader GetForumsByDayPostCount(int count, string posttableid)
        {
            ...
            string commandText = string.Format("SELECT DISTINCT `p`.`fid`, `f`.`name`, COUNT(`pid`) AS `posts` FROM `{0}posts{1}` `p` LEFT JOIN `{0}forums` `f` ON `p`.`fid`=`f`.`fid` WHERE `postdatetime`>='{2}' AND `invisible`=0 AND `posterid`>0 GROUP BY `p`.`fid`, `f`.`name` ORDER BY `posts` DESC LIMIT {3}", BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.ToString("yyyy-MM-dd"), count);
            return DbHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        public IDataReader GetUsersRank(int count, string posttableid, string type)
        {
            ...
            {
                case "posts":
                    commandText = string.Format("SELECT `username`, `uid`, `posts` FROM `{0}users` ORDER BY `posts` DESC, `uid` LIMIT {1}", BaseConfigs.GetTablePrefix, count);
                    break;
                case "digestposts":
                    commandText = string.Format("SELECT `username`, `uid`, `digestposts` FROM `{0}users` ORDER BY `digestposts` DESC, `uid` LIMIT {1}", BaseConfigs.GetTablePrefix, count);
                    break;
                case "thismonth":
                    commandText = string.Format("SELECT DISTINCT `poster` AS `username`, `posterid` AS `uid`, COUNT(pid) AS `posts` FROM `{0}posts{1}` WHERE `postdatetime`>='{2}' AND `invisible`=0 AND `posterid`>0 GROUP BY `poster`, `posterid` ORDER BY `posts` DESC LIMIT {3}", BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.AddDays(-30).ToString("yyyy-MM-dd"), count);
                    break;
                case "today":
                    commandText = string.Format("SELECT DISTINCT `poster` AS `username`, `posterid` AS `uid`, COUNT(pid) AS `posts` FROM `{0}posts{1}` WHERE `postdatetime`>='{2}' AND `invisible`=0 AND `posterid`>0 GROUP BY `poster`, `posterid` ORDER BY `posts` DESC LIMIT {3}", BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.ToString("yyyy-MM-dd"), count);
                    break;

                case "credits":
                    commandText = string.Format("SELECT `username`, `uid`, `credits` FROM `{1}users` ORDER BY `credits` DESC, `uid` LIMIT {0}", BaseConfigs.GetTablePrefix, count);
                    break;
                case "extcredits1":
                    commandText = string.Format("SELECT  `username`, `uid`, `extcredits1` FROM `{0}users` ORDER BY `extcredits1` DESC, `uid` LIMIT {0}", BaseConfigs.GetTablePrefix, count);
                    break;
                case "extcredits2":
                    commandText = string.Format("SELECT  `username`, `uid`, `extcredits2` FROM `{0}users` ORDER BY `extcredits2` DESC, `uid` LIMIT {0}", BaseConfigs.GetTablePrefix, count);
                    break;
                case "extcredits3":
                    commandText = string.Format("SELECT  `username`, `uid`, `extcredits3` FROM `{0}users` ORDER BY `extcredits3` DESC, `uid` LIMIT {0}", BaseConfigs.GetTablePrefix, count);
                    break;
                case "extcredits4":
                    commandText = string.Format("SELECT  `username`, `uid`, `extcredits4` FROM `{0}users` ORDER BY `extcredits4` DESC, `uid` LIMIT {0}", BaseConfigs.GetTablePrefix, count);
                    break;
                case "extcredits5":
                    commandText = string.Format("SELECT  `username`, `uid`, `extcredits5` FROM `{0}users` ORDER BY `extcredits5` DESC, `uid` LIMIT {0}", BaseConfigs.GetTablePrefix, count);
                    break;
                case "extcredits6":
                    commandText = string.Format("SELECT  `username`, `uid`, `extcredits6` FROM `{0}users` ORDER BY `extcredits6` DESC, `uid` LIMIT {0}", BaseConfigs.GetTablePrefix, count);
                    break;
                case "extcredits7":
                    commandText = string.Format("SELECT  `username`, `uid`, `extcredits7` FROM `{0}users` ORDER BY `extcredits7` DESC, `uid` LIMIT {0}", BaseConfigs.GetTablePrefix, count);
                    break;
                case "extcredits8":
                    commandText = string.Format("SELECT  `username`, `uid`, `extcredits8` FROM `{0}users` ORDER BY `extcredits8` DESC, `uid` LIMIT {0}", BaseConfigs.GetTablePrefix, count);
                    break;
                //case "oltime":
