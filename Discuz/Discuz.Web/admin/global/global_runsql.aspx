...
<%@ Page Language="c#" Inherits="Discuz.Web.Admin.runsql" Codebehind="global_runsql.aspx.cs" %>
...
<body>
    <div class="ManagerForm">
        ...
        <form id="Form1" method="post" runat="server">
            <uc1:PageInfo ID="info1" runat="server" Icon="information" Text="如果要一次运行多条SQL语句,语句之间请使用 --/* Discuz!NT SQL Separator */-- 进行分割即可." />
            <fieldset>
                <legend style="background: url(../images/icons/icon38.jpg) no-repeat 6px 50%;">执行SQL语句</legend>
                <table align="center" class="table1" cellspacing="0" cellpadding="4" width="100%">
                    <tr>
                        <td valign="baseline">
                            <uc1:TextareaResize ID="sqlstring" runat="server" controlname="sqlstring" cols="80" rows="10" is_replace="false"></uc1:TextareaResize>&nbsp;&nbsp;
                            <cc1:Button ID="RunSqlString" runat="server" Text=" 执行SQL语句 " Enabled="false"></cc1:Button>
                        </td>
                    </tr>
                </table>
            </fieldset>
        ...
        <script type="text/javascript">            
            document.getElementById("sqlstring_posttextarea").onchange = function(){ document.getElementById("RunSqlString").disabled = (document.getElementById("sqlstring_posttextarea").value == ""); }
        </script>
        <%=footer%>
    </div>
</body>
