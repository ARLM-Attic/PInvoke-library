...
#endif
    {
...
#if NET1
        #region 控件声明
        protected Discuz.Control.Button IntoDB;
        protected Discuz.Control.Button DelRec;
        protected Discuz.Control.DataGrid DataGrid1;
        protected Discuz.Control.Button DelTemplates;
        #endregion
#endif
        ...
        private void DelRec_Click(object sender, EventArgs e)
        {
            ...
            if (this.CheckCookie())
            {
                ...
                if (templateidlist != "")
                {
                    ...
                    int maxdbtemplateid = DatabaseProvider.GetInstance().GetMaxTemplateId();
                    ...
                    RemoveTemplateInDB(templateidlist, tempstr);
        ...
        private void CreateTemplateByDirectory(string directorypath)
        {
            ...
            if (this.CheckCookie())
            {
                ...
                App_Code.Globals.BuildTemplate(directorypath);
            }
        ...
        private void IntoDB_Click(object sender, EventArgs e)
        {
            ...
            if (this.CheckCookie())
            {
                ...
                if (templateidlist != "")
                {
                    ...
                    //base.RegisterStartupScript( "", "<script>alert('系统正在为入库的模板生成文件,这可能需要一些时间');</script>");
                    ...
                    foreach (DataRow dr in buildGridData().Select("templateid IN(" + templateidlist + ") AND templateid>" + maxdbtemplateid))
                    {
                        ...
                        try
                        {
                            DatabaseProvider.GetInstance().AddTemplate(dr["name"].ToString().Trim(),
