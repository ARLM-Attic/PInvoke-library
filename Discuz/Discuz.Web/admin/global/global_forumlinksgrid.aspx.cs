...
namespace Discuz.Web.Admin
{
...
#endif
    {
		...
		protected Discuz.Control.Button SaveFriend;
        #endregion
#endif
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if ((DNTRequest.GetString("displayorder").Trim() != "") && (DNTRequest.GetString("name").Trim() != ""))
            {
                ...
                try
                {
                    DatabaseProvider.GetInstance().AddForumLink(DNTRequest.GetInt("displayorder", 0),
                                            DNTRequest.GetString("name"),
                                            DNTRequest.GetString("url"),
                                            DNTRequest.GetString("note"),
        ...
        public void BindData()
        {
            ...
            DataGrid1.BindData(DatabaseProvider.GetInstance().GetForumLinks());
            #endregion
        }
        ...
        private void DelRec_Click(object sender, EventArgs e)
        {
            ...
            if (this.CheckCookie())
            {
                ...
                {
                    DatabaseProvider.GetInstance().DeleteForumLink(DNTRequest.GetString("delid"));
                    Discuz.Cache.DNTCache.GetCacheService().RemoveObject("/Forum/ForumLinkList");
                    AdminVistLogs.InsertLog(this.userid, this.username, this.usergroupid, this.grouptitle, this.ip, "删除友情链接", "删除友情链接,ID为: " + DNTRequest.GetString("id").Replace("0 ", ""));
