<%@ Page Language="C#" AutoEventWireup="false" CodeBehind="managemainmenu.aspx.cs" Inherits="Discuz.Web.Admin.managemainmenu" %>
	       ...
	       <Columns>
            ...
            <asp:BoundColumn DataField="system" HeaderText="系统菜单" readonly="true"></asp:BoundColumn>
              <asp:TemplateColumn HeaderText="操作">
                <ItemTemplate>
                    <a href="javascript:;" onclick="editMenu('<%# DataBinder.Eval(Container, "DataItem.id").ToString() %>','<%# DataBinder.Eval(Container, "DataItem.title").ToString() %>','<%# DataBinder.Eval(Container, "DataItem.defaulturl").ToString() %>');">编辑</a>&nbsp;
                    <%# DataBinder.Eval(Container, "DataItem.delitem").ToString()%>&nbsp;
                    <a href="managesubmenu.aspx?menuid=<%# DataBinder.Eval(Container, "DataItem.id").ToString() %>">管理子菜单</a>
                </ItemTemplate>
              </asp:TemplateColumn>
          </Columns>
	    </cc1:datagrid>
