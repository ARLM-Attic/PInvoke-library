<%@ Page Language="C#" AutoEventWireup="false" CodeBehind="managesubmenuitem.aspx.cs" Inherits="Discuz.Web.Admin.managesubmenuitem" %>
	   ...
	   <Columns>
        ...
        <asp:BoundColumn DataField="link" HeaderText="页面地址"><ItemStyle HorizontalAlign="left" /></asp:BoundColumn>
          <asp:TemplateColumn HeaderText="操作">
            <ItemTemplate>
                <a href="javascript:;" onclick="editMenu('<%# DataBinder.Eval(Container, "DataItem.id").ToString() %>','<%# DataBinder.Eval(Container, "DataItem.menutitle").ToString() %>','<%# DataBinder.Eval(Container, "DataItem.link").ToString() %>');">编辑</a>&nbsp;
                <a href="managesubmenuitem.aspx?menuid=<%=menuid%>&submenuid=<%=submenuid%>&pagename=<%=pagename%>&mode=del&id=<%# DataBinder.Eval(Container, "DataItem.id").ToString() %>" onclick='return confirm("您确认要删除此菜单项吗？")'>删除</a>
            </ItemTemplate>
          </asp:TemplateColumn>
      </Columns>
	</cc1:datagrid>
