<%@ Page Language="C#" AutoEventWireup="false" CodeBehind="managemenubackupfile.aspx.cs" Inherits="Discuz.Web.Admin.managemenubackupfile" %>
    ...
    <cc1:datagrid id="DataGrid1" runat="server">
	   <Columns>
	    <asp:TemplateColumn HeaderText="<input title='选中/取消' onclick='Check(this.form)' type='checkbox' name='chkall' id='chkall' />">
			...
			<ItemTemplate>
				<input id="backupname" onclick="checkedEnabledButton(this.form,'backupname','Delbackupfile')" type="checkbox" value="<%# DataBinder.Eval(Container, "DataItem.backupname").ToString() %>"	name="backupname" />
			</ItemTemplate>
		</asp:TemplateColumn>
	    <asp:BoundColumn DataField="backupdate" HeaderText="备份日期"></asp:BoundColumn>
          <asp:TemplateColumn HeaderText="操作">
            <ItemTemplate>
                <a href="managemenubackupfile.aspx?filename=<%# DataBinder.Eval(Container, "DataItem.backupname").ToString() %>" onclick='return confirm("您确认要将 <%# DataBinder.Eval(Container, "DataItem.backupdate").ToString() %> 的备份菜单恢复吗？\n\t注意：恢复将覆盖当前使用中的菜单！");'>恢复此备份</a>
            </ItemTemplate>
          </asp:TemplateColumn>
      </Columns>
	</cc1:datagrid>
