<%@ Page language="c#" Inherits="Discuz.Web.Admin.managesubmenu" Codebehind="managesubmenu.aspx.cs"  AutoEventWireup="false"%>
    ...
    <cc1:datagrid id="DataGrid1" runat="server">
				<Columns>
					...
					<asp:TemplateColumn HeaderText="操作"><HeaderStyle Width="30%" />
						<ItemTemplate>
						    <a href="javascript:;" onclick="editMenu('<%# DataBinder.Eval(Container, "DataItem.id").ToString() %>','<%# DataBinder.Eval(Container, "DataItem.menutitle").ToString() %>');">编辑</a>&nbsp;
                            <%# DataBinder.Eval(Container, "DataItem.delitem").ToString()%>&nbsp;
							<a href="managesubmenuitem.aspx?menuid=<%# DataBinder.Eval(Container, "DataItem.menuid").ToString() %>&submenuid=<%# DataBinder.Eval(Container, "DataItem.submenuid").ToString() %>&pagename=<%# Server.UrlEncode(DataBinder.Eval(Container, "DataItem.menutitle").ToString()) %>">管理子菜单项</a>
						</ItemTemplate>
					</asp:TemplateColumn>
				</Columns>
			</cc1:datagrid>
