...
namespace Discuz.Web.Admin
{
...
#endif
    {
        ...
        public void LoadCurrentForumInfo(int fid)
        {
            ...
            if (__foruminfo.Allowtag == 1) setting.Items[10].Selected = true;
        ...
        public void BindPower_Click(object sender, EventArgs e)
        {
            ...
            if (UserList.Text != "")
            {
                ...
                {
                    string uid = Users.GetUserID(adduser).ToString();
                    if (uid == "-1")
                        continue;
                    bool find = false;
        ...
        private string GetPermuserlist()
        {
            ...
            foreach (object o in SpecialUserList.GetKeyIDArray())
            {
                ...
                if (!find)
                {
                    result = Users.GetUserName(Convert.ToInt32(uid)).Trim() + "," + uid + "," + power + "|" + result;
                }
                row++;
            }
            if (result == "")
