<%@ Page language="c#" Inherits="Discuz.Web.Admin.identifymanage" Codebehind="forum_identifymanage.aspx.cs" %>
        ...
        <cc1:datagrid id="identifygrid" runat="server" OnPageIndexChanged="DataGrid_PageIndexChanged" IsFixConlumnControls="true" pagesize="10">
            <Columns>
                <asp:TemplateColumn HeaderText="<input title='选中/取消' onclick='Check(this.form)' type='checkbox' name='chkall' id='chkall' />">
                    ...
                    <ItemTemplate>
                        <input id="id" type="checkbox" onclick="checkedEnabledButton(this.form,'id','DelRec')" value="<%# DataBinder.Eval(Container, "DataItem.identifyid").ToString()%>" name="id" />
                        <%# identifygrid.LoadSelectedCheckBox(DataBinder.Eval(Container, "DataItem.identifyid").ToString())%>
                    </ItemTemplate>
                </asp:TemplateColumn>
                <asp:BoundColumn DataField="name" HeaderText="名称"><HeaderStyle width="80%" /></asp:BoundColumn>
                <asp:TemplateColumn HeaderText="图片">
                    <ItemTemplate>
                        <div id="layer<%# DataBinder.Eval(Container, "DataItem.identifyid").ToString()%>_a" onmouseover="showMenu(this.id,false)">
                            <%# PicStr(DataBinder.Eval(Container, "DataItem.filename").ToString(),20) %>
                        </div>
                        <div id="layer<%# DataBinder.Eval(Container, "DataItem.identifyid").ToString()%>_a_menu" style="display:none">
                            <%# PicStr(DataBinder.Eval(Container, "DataItem.filename").ToString())%>
                        </div>
                    </ItemTemplate>
                </asp:TemplateColumn>			
            </Columns>
