    ...
    public class AjaxPage : PageBase
    {
        public AjaxPage()
        {
            ...
            switch (type)
            {
                ...
                case "getgoodstradelog":
                    GetGoodsTradeLog(DNTRequest.GetInt("goodsid", 0), DNTRequest.GetInt("pagesize", 0), DNTRequest.GetInt("pageindex", 0), DNTRequest.GetString("orderby"), DNTRequest.GetInt("ascdesc", 1));
                    break;
                ...
                case "ajaxgetgoodsratelist":
                    GetGoodsRatesList(DNTRequest.GetInt("uid", 0), DNTRequest.GetInt("uidtype", 0), DNTRequest.GetInt("ratetype", 0), DNTRequest.GetString("filter"));
                    break;
                case "getmallhottags":
                    GetMallHotTags();
                    break;
        ...
        private void GetDebatePostPage()
        {
                ...
                Utils.MD5(forum.Password) != ForumUtils.GetCookie("forum" + forum.Fid.ToString() + "password"))
            {
                ...
                HttpContext.Current.Response.Redirect(BaseConfigs.GetForumPath + "showforum-" + forum.Fid.ToString() + config.Extname, true);
                return;
            }
        ...
        private void GetPhotoTags()
        {
            ...
            int photoid = DNTRequest.GetInt("photoid", 0);
            if (photoid <= 0)
            {
                return;
            }
            string filename = Utils.GetMapPath(string.Format("{0}cache/photo/{1}/{2}_tags.txt", BaseConfigs.GetForumPath, photoid / 1000 + 1, photoid));

            string tags = "";
            if (!File.Exists(filename))
            {
                AlbumPluginProvider.GetInstance().WritePhotoTagsCacheFile(photoid);
            }
        ...
        private void GetSpacePostTags()
        {
            ...
            SpacePluginProvider.GetInstance().GetSpacePostTagsCacheFile(postid);
        ...
        private void CheckRewriteName()
        {
            ...
            result = SpacePluginProvider.GetInstance().CheckSpaceRewriteNameAvailable(rewriteName).ToString();
        ...
        private StringBuilder GetNewPostXML(StringBuilder xmlnode, PostInfo postinfo, ForumInfo forum, TopicInfo topic, int postid)
        {
            ...
            xmlnode.AppendFormat("<enablespace>{0}</enablespace>", config.Enablespace);
            xmlnode.AppendFormat("<enablealbum>{0}</enablealbum>", config.Enablealbum);
            xmlnode.Append("</post>\r\n");
        ...
        public void GetAlbum()
        {
            ...
            int albumid = DNTRequest.GetQueryInt("albumid", 0);
            if (albumid < 1)
            {
                return;
            }
            AlbumPluginProvider.GetInstance().CreateAlbumJsonData(albumid);
            string builder = AlbumPluginProvider.GetInstance().GetAlbumJsonData(albumid);
            HttpContext.Current.Response.Clear();
