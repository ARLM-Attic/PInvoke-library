            ...
            switch (pageName)
            {
                case "showforum.aspx":
                    if (config.Aspxrewrite == 1)
                    {
                        ...
                        if (DNTRequest.GetQueryInt("forumid", 0) != 0)
                        {
                            sbUrl.Append("-" + DNTRequest.GetQueryString("forumid"));
                            if (DNTRequest.GetQueryInt("page", 0) != 0)
                            {
                                sbUrl.Append("-" + DNTRequest.GetQueryString("page"));
                            }
                        }
                        sbUrl.Append(GeneralConfigs.GetConfig().Extname);
                ...
                case "showtopic.aspx":
                    if (config.Aspxrewrite == 1)
                    {
                        StringBuilder sbUrl = new StringBuilder();
                        sbUrl.Append("../" + pageName.Substring(0, pageName.IndexOf('.')));
                        if (DNTRequest.GetQueryInt("topicid", 0) != 0)
                        {
                            sbUrl.Append("-" + DNTRequest.GetQueryInt("topicid", 0));
                            if (DNTRequest.GetQueryInt("page", 0) != 0)
                            {
                                sbUrl.Append("-" + DNTRequest.GetQueryString("page"));
                            }
