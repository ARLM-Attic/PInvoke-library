    ...
    public class ShowAlbumModule : ModuleBase
    {
...
#if NET1
            AlbumInfoCollection albumlist = DTOProvider.GetSpaceAlbumList(this.Module.Uid, 0, Utils.StrToInt(AlbumConfigs.GetConfig().MaxAlbumCount, 0), 1);
#else
        ...
        public override string OnMouduleLoad(string content)
        {
...
#if NET1
            PhotoInfoCollection photolist = DTOProvider.GetPhotoListByUserId(this.Module.Uid, albumid, photocount);
#else
            ...
            for (int i = 0; i < photolist.Count; i++)
            {
                ...
                //sbImgList.AppendFormat(@"{{""imgUrl"":""{0}""}},", BaseConfigs.GetForumPath + Globals.GetThumbnailImage(photo.Filename).Trim());     
            }
