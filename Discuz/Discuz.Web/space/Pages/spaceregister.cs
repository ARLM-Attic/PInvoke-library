    ...
    public class spaceregister : PageBase
    {
        ...
        protected override void ShowPage()
        {
            ...
            if (DNTRequest.IsPost())
            {
                ...
                if (page_err == 0)
                {
                    //string sql = "SELECT TOP 1 newid() AS row,[themeid],[directory] FROM dnt_spacethemes WHERE type<>0 ORDER BY row";
                    //DataRow dr = DbHelper.ExecuteDataset(CommandType.Text,sql).Tables[0].Rows[0];
                    ...
                    {
                        spaceconfiginfo.Rewritename = string.Empty;
                    }
                    //spaceconfiginfo.Rewritename = DNTRequest.GetString("rewritename");
                    ...
                    {
                        //sql = "UPDATE [" + BaseConfigs.GetTablePrefix + "users] SET [spaceid]=ABS([spaceid]) WHERE [uid]=" + userid;
                        DatabaseProvider.GetInstance().UpdateUserSpaceId(userid);
                        //sql += ";SELECT [spaceid] FROM [" + BaseConfigs.GetTablePrefix + "users] WHERE [uid]=" + userid;
                        //int spceid = (int)Database.ExecuteScalar(CommandType.Text, sql);
                        ...
                        Spaces.AddTab(tab);
                        //sql = "INSERT INTO [" + BaseConfigs.GetTablePrefix + "spacetabs] ([tabid],[uid],[displayorder], [tabname], [iconfile],[template])  VALUES(" + Spaces.GetNewTabId(userid) +  "," + userid + ",0,'默认','','template_25_75.htm');";
                        //int tabid = Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text,sql),0);
                        ...
                        ///添加最新主题到日志
#if NET1                        
                        MyTopicInfoCollection list = Topics.GetTopicsByUserId(userid, 1, config.Topictoblog, 0, 0);
#else
                        List<MyTopicInfo> list = Topics.GetTopicsByUserId(userid, 1, config.Topictoblog, 0, 0);
#endif                        
                        foreach (MyTopicInfo mytopic in list)
                        {
                            ...
                            if (post != null && post.Message.Trim() != string.Empty)
                            {
                                ...
                                string content = Posts.GetSpacePostMessage(post, new AttachmentInfo[0]);
                                spacepost.Category = "";
