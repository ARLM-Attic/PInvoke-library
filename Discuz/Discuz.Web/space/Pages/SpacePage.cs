...
namespace Discuz.Space.Pages
{
	...
	public class SpacePage : Page
	{
		...
		//private GeneralConfigInfo config;
		...
		public SpacePage()
		{
            ...
            //if (GeneralConfigs.GetConfig().Statstatus == 1)
            //{
		...
		private void OutputSpacePage()
		{
			...
			//detect the template whether exist, otherwise use the default template, if still not exist then throw an exception.
			//string templatePath = Utils.GetMapPath(root + "/space/skins/");


			//string mainTemplate = StaticFileProvider.GetContent(Utils.GetMapPath(root + "/space/skins/main.htm"));
...
//			if (currentuid == -1)
//			{
//				string screenName = DNTRequest.GetQueryString("name");
//				if (screenName != string.Empty)
//				{
//					spaceConfig = Spaces.GetSpaceConfigByScreenName(screenName);
//				}
//			}
...
#if NET1
			TabInfoCollection tc = Spaces.GetTabInfoCollectionByUserID(currentuid);
#else
...
//				return;
...
#if NET1
			ModuleInfoCollection mc = Spaces.GetModuleCollectionByTabId(defaulttabid, currentuid);
#else
			...
			//te.Init(mainTemplate, templatePath);
			...
			//ht.Add("footer", GeneralConfigs.GetConfig().SpaceFooterInfo);
            ht.Add("footer", SpaceActiveConfigs.GetConfig().SpaceFooterInfo);
            ht.Add("forumpath", BaseConfigs.GetForumPath);

            //string host = HttpContext.Current.Request.Url.Host;
            //int port = HttpContext.Current.Request.Url.Port;
            //string spaceurl = string.Format("http://{0}{1}{2}space/", host, (port == 80 ? "" : ":" + port), BaseConfigs.GetForumPath); //"http://" + host + (port == 80 ? "" : ":" + port) + BaseConfigFactory.GetForumPath + "?uid=" + spaceConfig.UserID;

            //if (config.Enablespacerewrite > 0 && spaceConfig.Rewritename != string.Empty)
            ////if (SpaceActiveConfigs.GetConfig().Enablespacerewrite > 0 && spaceConfig.Rewritename != string.Empty)
            //{
            ...
            //    spaceurl += "?uid=" + spaceConfig.UserID;
            //}
            //ht.Add("spaceurl", configspaceurlnopage + spaceurl);
...
//			processtime = new TimeSpan(DateTime.Now.Ticks).Subtract(begints).Duration().TotalSeconds;
//			ht.Add("processtime", processtime);


			//mainTemplate = mainTemplate.Replace("{modules}", template).Replace("IG_", "DS_").Replace("ig_", "ds_");

            string content = MainTemplate.Instance.GetHtml(ht);
			HttpContext.Current.Response.Clear();
            HttpContext.Current.Response.Write(content.Replace("IG_", "DS_").Replace("ig_", "dnt_"));//provide google module run
			HttpContext.Current.Response.End();
		...
		/// <returns></returns>
#if NET1         
		private string ParseModules(ModuleInfoCollection mc, string template)
#else
...
#endif
  		{
            ...
            for (int i = 0; i < mc.Count; i++)
            {
                ...
                else
                {
                    ...
                    //paneContent += desktopModule.GetModuleHtml();				
                }
            }
