...
namespace Discuz.Space.Entities
{
    public class MainTemplate : ISpaceTemplate
    {
        ...
        public string GetHtml(System.Collections.Hashtable ht)
        {
            ...
            {
                spacerssurl =
                    string.Format("spacerss-{0}{1}", ((SpaceConfigInfo) ht["config"]).UserID.ToString(),Discuz.Config.GeneralConfigs.GetConfig().Extname);
            }
            else
            {
                spacerssurl =
                    string.Format("rss.aspx?uid={0}&type=space", ((SpaceConfigInfo) ht["config"]).UserID.ToString());
            }
...
");
            ...
            if ((bool)ht["editable"])
            {
                //builder.Append("#parse(\"_editbar.htm\")");
