...
#if NET1
		private UserPrefCollection _userPrefCollection;
        public UserPrefCollection UserPrefCollection
#else
...
//		}
		...
		public virtual string GetModulePost(HttpContext httpContext)
		{
			...
			Hashtable saved = ModuleXmlHelper.LoadUserPrefsSaved(_module.UserPref);

			foreach (DictionaryEntry de in saved)
			{
				ht[de.Key] = de.Value;
			}
		...
		public string GetModuleHtml()
		{
			...
			//string moduleTemplate = StaticFileProvider.GetContent(Utils.GetMapPath(root + "/space/skins/module.htm")); //FileHelper.ReadFile(Utils.GetMapPath(root + "/space/templates/module.htm"));



			//TemplateEngine te = new TemplateEngine();
			Hashtable ht = new Hashtable();
//			ht.Add("{title}", _modulePref.TitleUrl == string.Empty ? _modulePref.Title : "<a href=\"" + _modulePref.TitleUrl + "\" target=\"_blank\">" + _modulePref.Title + "</a>");

//			UserPrefCollection upc = ModuleXmlHelper.LoadUserPrefs(_);
//			ht.Add("{meditbox}", GetModuleEditBox(modulefilepath));
//			
//			ht.Add("{modboxin}", GetModuleBoxin(modulefilepath));
			...
			if (_moduleContent != null && _moduleContent.Type != ModuleContentType.Url)
			{
				...
				foreach (UserPref up in _userPrefCollection)
				{
					...
					//alter the content
					//ht.Add("__UP_" + up.Name + "__", userprefvalue);
					...
					//moduleTemplate = moduleTemplate.Replace("_UP_" + up.Name + "__", userprefvalue);

				}
			}
//			ht.Add("__MODULE_ID__", _moduleID);
//			ht.Add("__TAB_ID__", _module.TabID);
//			ht.Add("__USERPREF_COUNT__", UserPrefCollection.Count);
...
//			ht.Add("moduleurl", _module.ModuleUrl);
//			ht.Add("moduleval", _module.Val);
			
/*
			html = moduleTemplate.Replace("{modboxin}", registScript.Append("</script>").ToString() + html);
			html = html.Replace("{meditbox}", editbox);
			html = html.Replace("{title}", moduleTitle);

*/

			//te.Init(moduleTemplate);
		...
		private string GetModuleEditBox()
		{
			...
			//string modulefilepath = Utils.GetMapPath(BaseConfigFactory.GetForumPath + "space/modules/" + _module.ModuleUrl);
		...
		private string GetModuleBoxin()
		{
			...
			//ModuleContent c = ModuleXmlHelper.LoadContent(modulefilepath);
			...
			return returnStr;//c.ContentHtml;
